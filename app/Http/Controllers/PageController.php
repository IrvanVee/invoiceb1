...
public function quoteshow($id){
    ...
    $detailquotations = DetailQuotation::where('quotation_id',$id)->get();
    // dd($detailquotation);
    // foreach ($detailquotations as $value) {
...
public function customerStoretambah(Request $request){
    $this->validate($request,[
        'instance' => 'required',
        'customer_name' => 'required',
        'contact' => 'required',
    ...
    return redirect()->back()->with('status', 'customer berhasil di tambah');
}
...
public function marketingUpdate($id, Request $request)
{
    $this->validate($request,[
        ...
        $marketing->instance = $request->instance;
